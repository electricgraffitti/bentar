    ...
    $.fn.gallery = function(opts) {
        ...
        function load(el) {
            ...
            image.onload = function() {
                ...
                // set info text & width
                var text = conf.template.replace("${title}", el.attr("title") || el.data("title"))
            ...
            image.onerror = function() {
                overlay.fadeIn()
                    .html("Cannot find image " + url);
            };

            image.src = url;

            if (conf.preload) {
                links.filter(":eq(" + (index - 1) + "), :eq(" + (index + 1) + ")")
                    .each(function() {
                    var img = new Image();
                    img.src = $(this)
                        .attr("href");
        ...
        if (conf.autohide) {
            overlay.hover(showEls, function() {
                els.fadeOut();
                close.hide();
            })
                .mousemove(showEls);
